...
class Led{
	...
	public:
		...
		~Led();
		/*Friend functions*/
		friend void printLedAttributes(Led led);
};
...
class MedicalLed:public Led{
	private:
		int wavelength;
	public:
		MedicalLed(float d, float v, float i, float w, int p){
			setDiameter(d);
			setVoltage(v);
			setCurrent(i);
			this->wavelength = w;
		...
		float mFactor(){
			return (this->getCurrent() * this->getDiameter() * (this->getWavelength()/100));
		}
		void getLedType(){
			if(this->nrOfPins > 3){
				printf("SMT LED\r\n");
			}
			else printf("Thourh Hole LED\r\n");
